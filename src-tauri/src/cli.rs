...
// *CLI*: a per-profile CLAUDE_CONFIG_DIR plus a per-profile credential injected at
// launch. Two auth methods are supported, detected by credential prefix:
//   • Subscription (Pro/Max/Team): an OAuth token (sk-ant-oat…, from
//     `claude setup-token`) → exported as CLAUDE_CODE_OAUTH_TOKEN.
//   • Console API (usage billing): an API key (sk-ant-api…) → exported as
//     ANTHROPIC_API_KEY.
// Both override the shared macOS Keychain slot, so profiles stay isolated and can
// run concurrently.
use serde::Serialize;
...
pub struct CliProfile {
    ...
    pub auth_kind: Option<String>,     // "subscription" | "console" (from the stored credential)
...
/// Which auth method a credential uses, by prefix: Console API keys are
/// `sk-ant-api…` → ANTHROPIC_API_KEY; everything else (subscription OAuth tokens
/// `sk-ant-oat…`) → CLAUDE_CODE_OAUTH_TOKEN.
pub fn credential_kind(cred: &str) -> &'static str {
    if cred.starts_with("sk-ant-api") { "console" } else { "subscription" }
}

/// The bash body of a profile launcher. Reads the credential from the Keychain at
/// run time (never bakes it into the file), picks the right auth env var by prefix
/// (sk-ant-api… → ANTHROPIC_API_KEY; else → CLAUDE_CODE_OAUTH_TOKEN), and execs
/// the real claude CLI.
...
pub fn render_launcher(name: &str, slug: &str, config_dir: &str, claude_bin: &str) -> String {
    ...
    format!(
        "#!/bin/bash\n\
         ...
         if [ -z \"$CRED\" ]; then\n\
         \x20 echo \"No saved credential for profile \\\"{name_e}\\\".\"\n\
         \x20 echo \"Open Claude Profiles and use \\\"Set credential\\\" for this profile.\"\n\
         \x20 exit 1\n\
         fi\n\
         case \"$CRED\" in\n\
...
/// Extract a usable credential from whatever the user pasted (a bare token/key, or
/// the full `claude setup-token` output). Accepts both subscription OAuth tokens
/// and Console API keys — prefer an sk-ant-* value; else the last non-empty line.
...
pub fn set_token(name: &str, token: &str) -> Result<(), String> {
    ...
    let clean = parse_setup_token_output(token)
        .ok_or("No credential found in the pasted text.")?;
    imp::kc_set(&slug, &clean)?;
    // Record which auth method this credential uses (non-secret) so the UI can
    // show it without ever reading the secret back out of the Keychain.
...
pub fn list() -> Vec<CliProfile> {
    ...
    for e in rd.flatten() {
        ...
        if p.extension().map_or(false, |x| x == "command") {
            ...
            let auth_kind = std::fs::read_to_string(cfg.join(".cli-auth-kind")).ok()
            ...
            out.push(CliProfile {
                ...
                account_email, auth_kind, data_size, created, last_active,
            });
        }
    }
...
mod tests {
    ...
    #[test]
    fn launcher_has_config_dir_cred_and_exec() {
        ...
        // both auth methods are handled, chosen by credential prefix
    ...
    #[test]
    fn credential_kind_detects_method() {
        assert_eq!(credential_kind("sk-ant-api03-XXXXXXXX"), "console");
        assert_eq!(credential_kind("sk-ant-oat01-XXXXXXXX"), "subscription");
        assert_eq!(credential_kind("anything-else"), "subscription");
    }
