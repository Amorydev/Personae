...
pub fn render_launcher_win(name: &str, config_dir: &str, claude_bin: &str) -> String {
    ...
    let bin = claude_bin; // path only; no `%` expected, but keep literal
    format!(
        "@echo off\r\n\
...
mod tests {
    ...
    fn win_launcher_sets_config_dir_and_calls_bin() {
        ...
        assert!(s.contains("call \"C:\\Users\\me\\AppData\\Roaming\\npm\\claude.cmd\" %*"));
        assert!(s.ends_with("\r\n"));
    }
    ...
    fn win_login_script_runs_auth_login_and_pauses() {
        ...
        assert!(s.contains("call \"C:\\bin\\claude.cmd\" auth login"));
